...
data {
  int<lower=1> n_groupings; // such as sampling event at revisit for species
  int<lower=1> n_total_samples; // total number of y-level samples
  int<lower=1> n_units; // total number of units that are revisited
  array[n_groupings] int<lower=1> n_revisits_per_grouping; // revisits per group
  int<lower=1> n_total_groupings; // total number of groupings
  array[n_total_groupings] int n_samples_per_grouping; // samples per grouping

  array[n_total_groupings] int z_obs; // detection in grouping
  array[n_total_samples] int a_obs; // detection in sample
  array[n_total_samples] int y; // number of detections in sample
  array[n_total_samples] int k_subsamples; // number of subsamples

  array[n_groupings] int unit_species_revisit_start; // indexing for grouping
  array[n_groupings] int unit_species_revisit_stop; // indexing for grouping
  array[n_groupings] int unit_species_dat_start; // indexing for grouping 
  array[n_groupings] int unit_species_dat_stop; // indexing for grouping 
  array[n_total_groupings] int revisit_start_in_unit; // indexing for grouping 
  array[n_total_groupings] int revisit_stop_in_unit; // indexing for grouping
  
  int<lower=1> m_beta; // number of coefficients
  int<lower=1> m_alpha; // number of coefficients
  int<lower=1> m_delta; // number of coefficients

  // psi-level coffients
  array[n_total_groupings] int jj_psi; 
  matrix[n_total_groupings, m_beta] x_psi;
  
  int<lower = 1> m_beta_star;
  matrix[n_units, m_beta_star] x_psi_star;

  // theta-level coffients
  array[n_total_samples] int jj_theta;
  matrix[n_total_samples, m_alpha] w_theta;
...
parameters {
  ...
  vector[n_total_groupings] logit_psi;
  vector[n_total_samples] logit_theta;
...
transformed parameters {
  ...
  vector[n_total_groupings] logit_psi_in;
  ...
  vector[n_total_samples] logit_theta_in;
...
model {
  vector[n_total_groupings] log_psi = log_inv_logit(logit_psi);
  vector[n_total_groupings] log1m_psi = log1m_inv_logit(logit_psi);
  
  vector[n_total_samples] log_theta = log_inv_logit(logit_theta);
  vector[n_total_samples] log1m_theta = log1m_inv_logit(logit_theta);
  ...
  target +=
    reduce_sum(unit_loop_p, n_revisits_per_grouping, grainsize,
               z_obs,
               a_obs,
               y,
               k_subsamples,
