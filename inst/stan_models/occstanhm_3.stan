...
#include ./functions/help_occstanhm_3.stan
}
data {
  ...
  int<lower=1> m_alpha; // number of row
  int<lower=1> m_delta; // number of row

  int<lower=1> n_beta; // number of coefficients
  int<lower=1> n_alpha; // number of coefficients
  int<lower=1> n_delta; // number of coefficients

  array[n_total_samples] int<lower=0> y; // detection during subsampling event
  array[n_total_samples] int<lower=0, upper=1> a_obs; // detection during sampling event
  array[n_total_samples] int<lower=0> k_subsamples; // detection during sampling event
  ...
  int<lower = 1> n_alpha_star;

  matrix[n_alpha_star, m_alpha_star] w_theta_star;
  ...
  real<lower=0> beta_psi_star_sd_prior; // prior values for gammas's SD
  real<lower=0> eta_psi; // prior for Chol. matrix

  // priors for theta
  matrix[m_alpha_star, m_alpha] alpha_theta_star_prior; // prior values for alpha_star
  real<lower=0> alpha_theta_star_sd_prior; // prior values for gammas's SD
  real<lower=0> eta_theta; // prior for Chol. matrix

  // priors for p
  matrix[m_delta_star, m_delta] delta_p_star_prior; // prior values for delta_star
  real<lower=0> delta_p_star_sd_prior; // prior values for gammas's SD
...
parameters {
  ...
  matrix[m_alpha, n_alpha_star] z_theta;
...
transformed parameters {
  ...
  // theta-level parameters
  matrix[n_alpha, m_alpha] alpha_theta;
  real<lower = 0> beta_theta_psi = eta_theta + (m_alpha - 2) / 2.0;
  ...
  // p-level
  shape_1_theta[1] = beta_theta_psi;
  shape_2_theta[1] = beta_theta_psi;

  for(k in 2:(m_alpha-1)) {
    beta_theta_psi -= 0.5;
    shape_1_theta[k] = k / 2.0;
    shape_2_theta[k] = beta_theta_psi;
  }
