...
## @knitr save_sim_data
saveRDS(sim_data,
        "../extdata/occstanhm_2_sim_data.rds")
...
## @knitr summarize_stan_data_revisit_species_2
...
saveRDS(stan_data,
        "../extdata/occstanhm_2_stan_data.rds")
saveRDS(fit,
        "../extdata/occstanhm_2_fit.rds")

saveRDS(fit_summary,
        "../extdata/occstanhm_2_fit_summary.rds")
...
beta_detail <-
  ...
  select(variable, unit, species, l95, prob, u95, psi_obs)
...
dat_rough_p <-
  ...
  filter(z_obs > 0) |>
  group_by(unit, unit_id, species, species_id) |>
  summarize(p_rough = mean(y), .groups = "drop")
...
p_details <-
  ...
  mutate(prob = plogis(median),
         l95 = plogis(q5),
         u95 = plogis(q95)) |>
  select(variable, unit, species, l95, prob, u95, p_rough) |>
...
test <- c("Omega_psi[1,1]", "Omega_p[1,2]")
...
fit_summary |>
  ...
  geom_linerange() +
  coord_flip() +
  facet_grid(vars(level), vars(species_col)) +
